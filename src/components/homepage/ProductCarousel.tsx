...
const ProductCarousel = () => {
  ...
  const { data: productData, isLoading } = useProducts(1, 4, "");
      ...
      {isLoading || !productData ? (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 max-w-6xl mx-4 md:mx-auto px-4">
          {[...Array(4)].map((_, index) => (
            <div
              key={index}
              className="bg-white shadow-md rounded-lg overflow-hidden p-3 md:p-4 animate-pulse"
            >
              <div className="relative h-40 md:h-60 w-full bg-gray-300" />
              <div className="pt-3 md:pt-4 space-y-2">
                <div className="h-4 bg-gray-300 rounded w-3/4"></div>
                <div className="h-3 bg-gray-300 rounded"></div>
                <div className="h-3 bg-gray-300 rounded w-1/2"></div>
                <div className="mt-3 md:mt-4 bg-gray-300 rounded h-8 w-full"></div>
              </div>
            </div>
          ))}
        </div>
          ...
          {products.map((product) => (
            ...
            >
              <div className="relative h-40 md:h-60 w-full">
                <Image
                  src={
                    product.productPictures?.[0]?.productPictureUrl ||
                    "/placeholder.jpg"
                  }
